...
(defun org-static-blog-assemble-tags ()
  ...
  (dolist (tag (org-static-blog-get-tag-tree))
    (org-static-blog-assemble-multipost-page
     ...
     (cdr tag)
     (concat "<h1 class=\"title\"><i class=\"fas fa-tag\"></i> " (car tag) "</h1>"))))
  ...
  (let ((post-filenames (cdr tag)))
    (setq post-filenames
	  (sort post-filenames (lambda (x y) (time-less-p (org-static-blog-get-date x)
						     (org-static-blog-get-date y)))))
    (concat "<div x-data=\"{display : false}\">\n"
            "<h1><span class=\"toggle-display\"><i @click=\"display = ! display\" class=\"fas fa-tag\"> </i></span>\n"
            "<span class=\"tags-title\"><a href=\"/tag-" (downcase (car tag)) ".html\">" (downcase (car tag)) "</a></span></h1>\n"
            "<div x-show.transition=\"display\">\n"
	    (apply 'concat (mapcar 'org-static-blog-get-post-summary post-filenames))
            "</div></div>\n" )))
...
;; 						     (org-static-blog-get-date y)))))
;;     (concat "<div x-data=\"{display : false}\">\n"
;;             "<h1><span class=\"toggle-display\" @click=\"display = ! display\"> |> </span><span class=\"tags-title\">\n"
;;             "<a href=\"tag-"(downcase (car tag)) ".html\">"
;;             (downcase (car tag)) "</a>"
;;             "</span></h1>\n<div x-show.transition=\"display\">\n"
;;             (apply 'concat (mapcar 'org-static-blog-get-post-summary post-filenames))
;;             "</div></div>\n" )))
